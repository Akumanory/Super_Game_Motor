...
string CStringOp::tolowercase(const string& str)
{
    ...
    while ( ix < nSize )
    {
        if ( str[ix] < 0 && (ix+1 < nSize) )
        //--if ( !isascii(str[ix]) && (ix+1 < nSize) )
        {
...
bool CStringOp::IsPunc(const string& str)
{
...
{
    myAssertPlus(!sep.empty(), "split function cannot be empty!");
    std::vector<std::string> ret;
