...
Texture::Texture(const wchar_t* full_path)
    ...
    HRESULT res = DirectX::LoadFromWICFile(full_path, DirectX::WIC_FLAGS_IGNORE_SRGB, nullptr, image_data) if (FAILED(res)) {
...
Texture::Texture(const Rect& size, Texture::Type type)
    : Resource(L"") {
    ...
    sampler_desc.Filter = D3D11_FILTER_ANISOTROPIC;
    sampler_desc.MinLOD = 0.0f;
    sampler_desc.MaxLOD = (FLOAT)1.0f;

    HRESULT res = GraphicsEngine::get()->getRenderSystem()->m_d3d_device->CreateSamplerState(&sampler_desc, &m_sampler_state);
    if (FAILED(res)) {
        throw std::exception("Texture not created successfully");
    }
