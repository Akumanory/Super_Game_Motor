...
bool GameObject::CollisionIntersects(BoundingSphere& bound)
{
	return bounding_sphere.Intersects(bound);
}

BoundingSphere& GameObject::GetCollisionSphere()
{
	return bounding_sphere;
}

void GameObject::BoundToIntersected(GameObject& object, XMFLOAT3 offset)
{
	if (collide)
	{
		DisableCollision();
		bounding_sphere = BoundingSphere(pos, 0.0f);

		this->offset = offset;

		SetPosition(
			object.GetPositionFloat3().x + this->offset.x,
			object.GetPositionFloat3().y + this->offset.y,
			object.GetPositionFloat3().z + this->offset.z
		);
	}
	else
	{
...
void GameObject::BindOnIntersect(GameObject& child)
{
	if (this->CollisionIntersects(child.GetCollisionSphere()))
	{
		
		//Logs::Debug("Intesected");
		child.SetRotation(this->GetRotationVector());


		child.BoundToIntersected(
			*this,
			XMFLOAT3(
				child.GetPositionFloat3().x - this->GetPositionFloat3().x,
				child.GetPositionFloat3().y - this->GetPositionFloat3().y,
				child.GetPositionFloat3().z - this->GetPositionFloat3().z
			)
		);
	}
